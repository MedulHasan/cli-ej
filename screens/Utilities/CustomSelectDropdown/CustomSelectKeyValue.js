...
const CustomSelectKeyValue = ({ filterArray, setValue, dropdownRef }) => {
    let dropdownData;
    if (Array.isArray(filterArray)) {
        dropdownData = [...filterArray];
    }
    ...
    return (
        <SelectDropdown
            ...
            onSelect={(selectedItem) => {
                if (selectedItem.value === `Select One`) return setValue("");
                else return setValue(selectedItem.value);
            }}
            defaultButtonText={dropdownData[0].label}
...
export default CustomSelectKeyValue;
